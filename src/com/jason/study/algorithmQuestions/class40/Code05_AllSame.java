...
public class Code05_AllSame {
    ...
    // right 可能性 : N
    // midV 可能性 : arr中的最大值！
    public static int process(int[] arr, int left, int value, int right) {
        //先向两边扩
        ...
        //没搞完，向左变或者向右变
        int p1 = left == -1 ? Integer.MAX_VALUE : process(arr, left - 1, arr[left], right);
        int p2 = right == arr.length ? Integer.MAX_VALUE : process(arr, left, arr[right], right + 1);
        return Math.min(p1, p2) + 1/* 本次变的代价 */;
    }
